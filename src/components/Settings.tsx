...
export const Settings: React.FC<SettingsProps> = ({ onBack }) => {
  const { startDay, theme, setStartDay, setTheme } = useConfig();
    ...
    >
      ...
      <div className="space-y-6">
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          <h3 className="text-lg font-semibold mb-4 text-white">Día de inicio de semana</h3>
          <select
            value={startDay}
          ...
          >
            {days.map((day) => (
              <option key={day} value={day} className="text-white bg-gray-700">
                {day}
              </option>
            ))}
          </select>
